...
export class WindowOpener extends React.Component {
  ...
  // opens a child
  onClickHandler() {
    const { url, name, opts, croppedImage, allImages } = this.props;
    ...
    setTimeout(() => {
      ...
      popup.opener.postMessage(
        { message: "Opening Popup", croppedImage: croppedImage, allAssets: allImages},
        "*"
      );
    }, 0);
...
WindowOpener.defaultProps = {
  name: "Popup",
  opts: `dependent=${1}, alwaysOnTop=${0}, alwaysRaised=${1}, width=${900}, height=${590}, left=${250}, top=${120}`,
};
