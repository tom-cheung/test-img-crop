...
function makeAspectCrop(crop, imageWidth, imageHeight) {
  // console.log('crop', isNaN(crop.aspect), crop.aspect);
  ...
  // console.log('comlete', completeCrop);
  return completeCrop;
}
...
class ReactCrop extends PureComponent {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (this.imageRef && prevProps.crop !== crop && crop.aspect
  ...
  onComponentMouseTouchDown = e => {
    e.stopPropagation();
    const {
      crop, disabled, locked, keepSelection, onChange
    } = this.props;
    ...
    onChange(convertToPixelCrop(nextCrop, width, height),
  ...
  onDocMouseTouchMove = e => {
    e.stopPropagation();
    const {
      crop, disabled, onChange, onDragStart
    } = this.props;
  ...
  onComponentKeyDown = e => {
    e.stopPropagation();
    const {
      crop, disabled, onChange, onComplete
    } = this.props;
  ...
  onDocMouseTouchEnd = e => {
    e.stopPropagation();
    const {
      crop, disabled, onComplete, onDragEnd
    } = this.props;
    ...
    if (this.mouseDownOnCrop) {
      ...
      onDragEnd(e);
      onComplete(convertToPixelCrop(crop, width, height),
  ...
  getNewSize() {
    const {
      crop, minWidth, maxWidth, minHeight, maxHeight
    } = this.props;
    const { evData } = this;
    const { width, height } = this.mediaDimensions;
  ...
  calculateOffsetFromFocus() {
    ...
    let compStyle = window.getComputedStyle(container);
    let height = parseFloat(compStyle.getPropertyValue('height'));
    let width = parseFloat(compStyle.getPropertyValue('width'));
    const offsetX = width * (parseFloat(container.getAttribute('data-focus-x')) / 2 + 0.5);
    const offsetY = height * (parseFloat(container.getAttribute('data-focus-y')) / -2 + 0.5);
  ...
  updateRetinaPosition = (offsets) => {
    ...
    if (retina) {
      retina.style.top = `${(offsets.offsetY - 20)}px`;
      retina.style.left = `${(offsets.offsetX - 20)}px`;
    }
  }
  ...
  createCropSelection() {
    const {
      disabled, locked, renderSelectionAddon, ruleOfThirds, crop
    } = this.props;
    ...
    return (
      <div
        ...
        data-focus-x="0"
        data-focus-y="0"
        className="ReactCrop__crop-selection ui-widget-content draggable"
  ...
  render() {
    ...
    const cropSelection = isCropValid(crop)
      && this.componentRef ? this.createCropSelection() : null;
...
export {
  ReactCrop as default, ReactCrop as Component, makeAspectCrop, containCrop
};
