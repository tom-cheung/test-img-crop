...
export default class FocusedImage {
  constructor(
    imageNode,
    options = {}
  ) {
  ...
  startListening() {
    ...
    if (this.options.updateOnContainerResize) {
      ...
      object.addEventListener('load', () => object.contentDocument.defaultView.addEventListener('resize', () => this.debounceApplyShift()));
  ...
  calcShift(
    ...
    let focusOffset = focus - containerCenter; // Calculate difference between focus point & center
